...
void StoreManager::LoadStore(){
	//sLog->outInfo("server.loading", ">> Store loading...");
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Store loading...");
	LoadCategories();
	LoadItems();
	//TC_LOG_INFO("server.loading", ">> Store successfully loaded");
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Store successfully loaded");
}
