    ...
    public:
        ...
        struct boss_garajalAI : public BossAI
        {
            ...
            void JustDied(Unit* attacker)
            {
				...
				//pInstance->DoCompleteAchievement(uint32() achievement);
				pInstance->DoCompleteAchievement(IsHeroic() ? ACHIEVEMENT_HEROIC_GARAJAL : ACHIEVEMENT_NORMAL_GARAJAL);
        ...
        struct mob_shadowy_minionAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                ...
                if (me->GetEntry() == NPC_SHADOWY_MINION_REAL)
                {
                    ...
                    if (Creature* spirit = me->SummonCreature(NPC_SHADOWY_MINION_SPIRIT, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 40000))
                    {
