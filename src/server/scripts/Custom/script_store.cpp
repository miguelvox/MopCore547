...
public:
	...
	bool displayInitialMenu(Player* player, Creature* creature) {
		...
		std::string msg = "Tienes " + std::to_string(coins) + " store coins";
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
	{
		...
		if (action >= MAX_ENTRY && action != 9000000) {
			...
			case Maelstrom::StoreManager::PURCHASE_RESULT_ERROR: {
				player->MonsterSay("Ha ocurrido un error ", LANG_UNIVERSAL, 0);
				player->PlayerTalkClass->ClearMenus();
			...
			case Maelstrom::StoreManager::PURCHASE_RESULT_NOT_ENOUGH_COINS: {
				...
				player->MonsterSay("No tienes suficientes monedas para comprar este item. ", LANG_UNIVERSAL, 0);
				player->PlayerTalkClass->ClearMenus();
				player->CLOSE_GOSSIP_MENU();
				return false;
			}
			...
			case Maelstrom::StoreManager::PURCHASE_RESULT_NOT_ENOUGH_FREE_SLOTS: {
				...
				player->MonsterSay("No tienes espacio en tu inventario para comprar este objeto. ", LANG_UNIVERSAL, 0);
				player->PlayerTalkClass->ClearMenus();
				player->CLOSE_GOSSIP_MENU();
				return false;
			}
			...
			case Maelstrom::StoreManager::PURCHASE_RESULT_SUCCESS: {
				...
				player->MonsterSay("Gracias por tu compra.. ", LANG_UNIVERSAL, 0);
				player->PlayerTalkClass->ClearMenus();
				player->CLOSE_GOSSIP_MENU();
				return true;
			}
		...
		//On récupère les points boutiques du joueur et on les affiche
		uint32 coins = Maelstrom::sStoreMgr->GetAccountCoins(player->GetSession()->GetAccountId());
		std::string msgCoins = "Tienes " + std::to_string(coins) + " store coins";
		...
		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Atras", GOSSIP_SENDER_MAIN, 9000000);
